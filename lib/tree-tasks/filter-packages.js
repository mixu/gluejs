...
module.exports = function(tree) {
  ...
  function traverse(packageObj) {
    // sort by length
    packageObj.files = packageObj.files.sort(function(a, b) { return a.length - b.length; } );
    var rules = [], raw,
     ...
     if(files) {
      ...
      files.forEach(function(relpath) {
        if(/\/package.json$/.test(relpath)) {
          var p = JSON.parse(fs.readFileSync(relpath));
        ...
        if(/\/\.npmignore$/.test(relpath) || /\/\.gitignore$/.test(relpath)) {
          // ported from isaacs/fstream-ignore
          var set = fs.readFileSync(relpath).toString().split(/\r?\n/);
      ...
      if(rules.length > 0) {
        packageObj.files = files.filter(function(name) {
          var result = match(rules, name);
