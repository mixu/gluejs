...
function getTasks(filename, opts) {
  ...
  if (isMain && opts.command) {
    tasks.concat(commandsToTasks(Array.isArray(opts.command) ? opts.command : [opts.command]));
  }
  if (isMain && opts.transform) {
    tasks.concat(transformsToTasks(Array.isArray(opts.transform) ? opts.transform :
      [opts.transform]));
  }
  if (opts['global-command']) {
    tasks.concat(commandsToTasks(Array.isArray(opts['global-command') ? opts['global-command' :
      [opts['global-command']));
  }
  if (opts['global-transform']) {
   tasks.concat(transformsToTasks(Array.isArray(opts['global-transform']) ? opts['global-transform'] :
    [opts['global-transform']]));
  }
