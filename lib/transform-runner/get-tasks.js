...
function commandsToTasks(filename, commands) {
  var tasks = [],
...
function transformsToTasks(filename, transforms) {
  var tasks = [];
...
function getTasks(filename, opts) {
  ...
  if (isMain && opts.command) {
    tasks = tasks.concat(commandsToTasks(filename, asArray(opts.command)));
  }
  if (isMain && opts.transform) {
    tasks = tasks.concat(transformsToTasks(filename, asArray(opts.transform)));
  }
  if (opts['global-command']) {
    tasks = tasks.concat(commandsToTasks(filename, asArray(opts['global-command'])));
  }
  if (opts['global-transform']) {
    tasks = tasks.concat(transformsToTasks(filename, asArray(opts['global-transform'])));
  }
