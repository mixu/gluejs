...
Renderer.prototype.render = function(done){
  ...
  var data = '(function(){'
    ...
    + Object.keys(this.code).reduce(function(str, moduleName) {
      return str + self._render(moduleName, self.code[moduleName]);
    }, '')
    + this.paths.reduce(function(str, filepath) {
      var relpath = filepath.replace(new RegExp('^'+self.options.basepath), '');
      return str + self._render(relpath, fs.readFileSync(filepath, 'utf8'));
    }, '')
...
Renderer.prototype._render = function(moduleName, source) {
