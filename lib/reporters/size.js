  ...
  emitter.once('done', function() {
    process.nextTick(function() {
      ...
      files.forEach(function(file) {
        ...
        var row = ['  ', style(path.dirname(relpath) + '/', 'gray') + path.basename(relpath), ' ', bytes(statBefore[file].size),
      ...
      if (totalAfter > 0) {
        process.stderr.write('Total size: ' + bytes(totalAfter) + (totalAfter > 0 ? compare(total, totalAfter).join('') : '') + '\n');
      } else {
