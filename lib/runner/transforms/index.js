...
var getTasks = require('./get-tasks.js'),
    ...
    detectiveDependencies = require('../../list/amd2.js');
...
RunQueue.prototype.add = function(filename) {
  ...
  if (self.hasCached(filename)) {
    ...
    this.emit('file-done', filename, this.cache.file(filename).path(this.key('cacheFile')));
    return;
  }
  // Apply exclusions
  ...
  if (this.ignore) {
    ...
    if (isIgnored) {
      ...
      this.emit('file-done', filename, self.ignoreFile);
      return;
    }
  }
...
RunQueue.prototype.parseAndUpdateDeps = function(filename, cacheFile, done) {
  ...
  detectiveDependencies(filename, cacheFile, this.opts.ignore, this.dependencyCache,
    ...
    self.emit('file-done', filename, cacheFile);
    done();
  });
};
