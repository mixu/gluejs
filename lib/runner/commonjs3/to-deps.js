...
function isIgnored(filename, ignoredDeps) {
  return ignoredDeps.some(function(expr) { return !!filename.match(expr); });
}

module.exports = function(files, basepath, main, ignoredDeps) {
  var deps = [];
  var id = 1;
  ...
  */
  ...
  files.forEach(function(file) {
    ...
    Object.keys(file.deps).forEach(function(rawDep) {
      var resolvedDep = file.deps[rawDep];
      // convert deps hash to ids
      if (idLookup[resolvedDep]) {
        dhash[rawDep] = idLookup[resolvedDep];
      } else if (!isIgnored(resolvedDep, ignoredDeps)) {
        log.warn('Dependency not found', rawDep, resolvedDep);
      }
    });
